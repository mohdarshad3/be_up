...
/*global token */
angular.module('Admin').controller('messageCtrl', function ($scope, $modal, $rootScope, $location, $state, $localStorage, CONFIG, dataService, AUTH_EVENTS, $timeout,$stateParams,$confirm,userService,messageService,toastr,productCategoryService) {
    ...
    $scope.getUserInboxList = function () {

        $scope.userId = $stateParams.userId ? $stateParams.userId : null;
        messageService.getUserInboxList($scope.currentPage,$scope.userId, function (err, data) {
            if (!err) {
                $scope.itemsPerPage = CONFIG.limit;
    ...
    $scope.getUserSentBoxList = function () {

        $scope.userId = $stateParams.userId ? $stateParams.userId : null;
        messageService.getUserSentBoxList($scope.currentPage,$scope.userId, function (err, data) {
            if (!err) {
                $scope.itemsPerPage = CONFIG.limit;
                $scope.totalItems = data.total;
                $scope.results = data.docs;
